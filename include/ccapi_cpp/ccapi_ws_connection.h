 ...
 * This class represents a TCP socket connection for the websocket API.
 */
class EpollWs CCAPI_FINAL {
 public:
  EpollWs(std::string url, std::string group, std::vector<Subscription> subscriptionList, std::map<std::string, std::string> credential,
  ...
  std::string toString() const {
    ...
    std::string output = "WsConnection [id = " + id + ", url = " + url + ", group = " + group + ", subscriptionList = " + ccapi::toString(subscriptionList) +
                         ", credential = " + ccapi::toString(shortCredential) + ", status = " + statusToString(status) +
                         ", headers = " + ccapi::toString(headers) + ", hostHttpHeaderValue = " + ccapi::toString(hostHttpHeaderValue) +
                         ", path = " + ccapi::toString(path) + ", host = " + ccapi::toString(host) + ", port = " + ccapi::toString(port) + "]";
    return output;
  }
  enum class Status {
    UNKNOWN,
  ...
  std::map<std::string, std::string> headers;
  std::map<std::string, std::string> credential;
  std::unique_ptr<emumba::connector::websocket::client> _socket;
