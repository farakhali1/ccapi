...
// Rakurai Changes
  ...
  void ping(std::shared_ptr<WsConnection> wsConnectionPtr, boost::beast::string_view payload, ErrorCode& ec) {
    ...
    //   stream.async_ping("", [that = this, wsConnectionPtr](ErrorCode const& ec) { that->wsConnectionPendingPingingByConnectionIdMap[wsConnectionPtr->id] =
    //   false; }); this->wsConnectionPendingPingingByConnectionIdMap[wsConnectionPtr->id] = true;
    // }
  }
