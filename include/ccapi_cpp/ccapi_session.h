 ...
 public:
...
#endif
  {
    if (!this->serviceContextPtr) {
      ...
      _ws_rate_limit_timer = std::make_shared<emumba::connector::epoll_timer>(_io_handler);
      _http_rate_limit_timer = std::make_shared<emumba::connector::epoll_timer>(_io_handler);
...
// #endif
    ...
    for (const auto& x : this->serviceByServiceNameExchangeMap) {
      ...
      for (const auto& y : x.second) {
        ...
        service_Ptr->setIOHandlerAndTimer(_io_handler, _ws_rate_limit_timer, _http_rate_limit_timer);
